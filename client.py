...
class MainWindow(CTk):
    def __init__(self):
        ...
        self.message_entry = CTkEntry(self, placeholder_text='Сообщить о вирусе', height=40)
        ...
        self.add_message("Демонстрация:",
                 CTkImage(Image.open('player.jpg'), size=(300, 300)))

        try:
            self.sock = socket(AF_INET, SOCK_STREAM)
            self.sock.connect(('localhost', 8080))
            hello = f"TEXT@{self.username}@[SYSTEM] {self.username} добро пожаловать!\n"
            self.sock.send(hello.encode('utf-8'))
            threading.Thread(target=self.recv_message, daemon=True).start()
        except Exception as e:
            self.add_message(f"Заархивировано: {e}")
    ...
    def toggle_show_menu(self):
        if self.is_show_menu:
            ...
            self.label = CTkLabel(self.menu_frame, text="Н¿К")
            self.label.pack(pady=30)
            self.entry = CTkEntry(self.menu_frame, placeholder_text="В@ш ф@йл...")
            self.entry.pack()
            # Кнопка збереження
            self.save_button = CTkButton(self.menu_frame, text="сохронить", command=self.save_name)
            self.save_but
    ...
    def save_name(self):
        ...
        if new_name:
            ...
            self.add_message(f"Ваш новий ф@йл: {self.username}")
    ...
    def add_message(self, message, img=None):
        message_frame = CTkFrame(self.chat_field, fg_color='#D21F3C') #контур картинки и сообщений
        ...
        if msg_type == "TEXT":
            ...
            if len(parts) >= 4:
                ...
                try:
                    ...
                    self.add_message(f"{author} Отоброжение камеры: {filename}",img=ctk_img)
                except Exception as e:
                    self.add_message(f"ØшNδк@ к@WΞры: {e}")
        else:
    ...
    def open_image(self):
        ...
        try:
            ...
            self.sock.sendall(data.encode())
            self.add_message("",img=CTkImage(light_image=Image.open(file_name),size=(300, 300)))
        except Exception as e:
            self.add_message(f"¡Не удалось получить запись¡: {e}")
